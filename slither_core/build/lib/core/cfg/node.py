...
from slither_core.all_exceptions import SlitherException
from slither_core.core.declarations import Contract, Function, FunctionContract
...
from slither_core.slithir.convert import convert_expression
...
from slither_core.slithir.utils.utils import RVALUE
...
if TYPE_CHECKING:
    from slither_core.slithir.variables.variable import SlithIRVariable
    from slither_core.core.compilation_unit import SlitherCompilationUnit
    from slither_core.utils.type_helpers import (
        InternalCallType,
        HighLevelCallType,
        LibraryCallType,
        LowLevelCallType,
...
class NodeType(Enum):
    ...
    # STARTLOOP node source mapping points to the entire loop "body"
    STARTLOOP = "BEGIN_LOOP"
...
class Node(SourceMapping):  # pylint: disable=too-many-public-methods
    ...
    def __init__(
        ...
        self._phi_origins_state_variables: Dict[str,
                                                Tuple[StateVariable, Set["Node"]]] = {}
        self._phi_origins_local_variables: Dict[str,
                                                Tuple[LocalVariable, Set["Node"]]] = {}
        ...
        self._slithir_vars: Set[
            Union["SlithIRVariable", ReferenceVariable,
    ...
    def can_reenter(self, callstack: Optional[List[Union[Function, Variable]]] = None) -> bool:
        ...
        from slither_core.slithir.operations import Call
    ...
    def remove_father(self, father: "Node") -> None:
        ...
        """
        self._fathers = [
            x for x in self._fathers if x.node_id != father.node_id]
    ...
    def dominance_exploration_ordered(self) -> List["Node"]:
        ...
        to_explore = sorted(list(self.dominator_successors),
        ...
        to_explore += sorted(list(self.dominance_frontier),
                             key=lambda x: x.node_id)
        return to_explore
    ...
    def add_phi_origin_local_variable(self, variable: LocalVariable, node: "Node") -> None:
        if variable.name not in self._phi_origins_local_variables:
            ...
            self._phi_origins_local_variables[variable.name] = (
                variable, set())
        ...
        for ir in self.irs:
            ...
            if isinstance(ir, LowLevelCall):
                ...
                self._low_level_calls.append(
                    (ir.destination, str(ir.function_name.value)))
    ...
    def update_read_write_using_ssa(self) -> None:
        ...
        self._vars_written += [
            v_ for v_ in vars_written if v_ and v_ not in self._vars_written]
        self._state_vars_written = [
            v for v in self._vars_written if isinstance(v, StateVariable)]
        self._local_vars_written = [
            v for v in self._vars_written if isinstance(v, LocalVariable)]
