...
from slither_core.core.expressions.index_access import IndexAccess
from slither_core.core.expressions.literal import Literal
from slither_core.core.expressions.new_array import NewArray
from slither_core.core.expressions.new_contract import NewContract
from slither_core.core.expressions.tuple_expression import TupleExpression
from slither_core.core.expressions.unary_operation import UnaryOperation
from slither_core.core.solidity_types import ArrayType, ElementaryType, TypeAlias, UserDefinedType
...
from slither_core.slithir.tmp_operations.argument import Argument
...
class ExpressionToSlithIR(ExpressionVisitor):
    ...
    def __init__(self, expression: Expression, node: "Node") -> None:
        from slither_core.core.cfg.node import NodeType  # pylint: disable=import-outside-toplevel
    ...
    def _post_binary_operation(self, expression: BinaryOperation) -> None:
        ...
        if expression.type in _signed_to_unsigned:
            ...
            conv_left = TypeConversion(
                new_left, left, ElementaryType("int256"))
            ...
            if expression.type != BinaryOperationType.RIGHT_SHIFT_ARITHMETIC:
                ...
                conv_right = TypeConversion(
                    new_right, right, ElementaryType("int256"))
            ...
            conv_final = TypeConversion(
                val, new_final, ElementaryType("uint256"))
            val.set_type(ElementaryType("uint256"))
            conv_final.set_expression(expression)
            self._result.append(conv_final)
        else:
            operation = Binary(
                val, left, right, _binary_to_binary[expression.type])
            operation.set_expression(expression)
            self._result.append(operation)
    ...
    def _post_call_expression(self, expression: CallExpression) -> None:
        ...
        if isinstance(called, Function):
            ...
            internal_call = InternalCall(
                called, len(args), val, expression.type_call)
        ...
        # yul things
            ...
            val = TemporaryVariable(self._node)
            var = Assignment(val, SolidityVariableComposed(
                "msg.sender"), ElementaryType("uint256"))
            self._result.append(var)
            set_val(expression, val)
        elif called.name == "origin()":
            val = TemporaryVariable(self._node)
            var = Assignment(val, SolidityVariableComposed(
                "tx.origin"), ElementaryType("uint256"))
            self._result.append(var)
            set_val(expression, val)
            ...
            val = TemporaryVariable(self._node)
            var = TypeConversion(val, SolidityVariable(
                "this"), ElementaryType("address"))
            ...
            val = TemporaryVariable(self._node)
            var = Assignment(val, SolidityVariableComposed(
                "msg.value"), ElementaryType("uint256"))
            self._result.append(var)
            set_val(expression, val)
            ...
            message_call = TmpCall(called, len(
                args), val, expression.type_call)
    ...
    def _post_literal(self, expression: Literal) -> None:
        ...
        cst = Constant(expression.value, expression_type,
    ...
    def _post_member_access(self, expression: MemberAccess) -> None:
            ...
            if isinstance(expression.expression.called, Identifier):
                if expression.expression.called.value == SolidityFunction("type()"):
                    ...
                    if isinstance(type_expression_found, ElementaryTypeNameExpression):
                        ...
                        assert isinstance(
                            type_expression_found_type, ElementaryType)
        ...
        if isinstance(expr, Contract):
            ...
            if expression.member_name in expr.file_scope.user_defined_types:
                set_val(
                    expression, expr.file_scope.user_defined_types[expression.member_name])
                return
            ...
            if expression.member_name in expr.custom_errors_as_dict:
                set_val(
                    expression, expr.custom_errors_as_dict[expression.member_name])
                return
    ...
    def _post_tuple_expression(self, expression: TupleExpression) -> None:
        all_expressions = [
            get(e) if e else None for e in expression.expressions]
    ...
    def _post_type_conversion(self, expression: expressions.TypeConversion) -> None:
        ...
        val = TemporaryVariable(self._node)
        expression_type = expression.type
        assert isinstance(expression_type, (TypeAlias,
                          UserDefinedType, ElementaryType))
    ...
    def _post_unary_operation(self, expression: UnaryOperation) -> None:
        ...
        if expression.type in [UnaryOperationType.BANG, UnaryOperationType.TILD]:
            ...
            operation = Binary(value, value, Constant(
                "1", value.type), BinaryType.ADDITION)
            operation.set_expression(expression)
            self._result.append(operation)
            set_val(expression, value)
        elif expression.type in [UnaryOperationType.MINUSMINUS_PRE]:
            operation = Binary(value, value, Constant(
                "1", value.type), BinaryType.SUBTRACTION)
            operation.set_expression(expression)
            self._result.append(operation)
            set_val(expression, value)
            ...
            lvalue = TemporaryVariable(self._node)
            operation = Binary(lvalue, Constant(
                "0", value.type), value, BinaryType.SUBTRACTION)
            operation.set_expression(expression)
            self._result.append(operation)
            set_val(expression, lvalue)
        else:
            raise SlithIRError(
                f"Unary operation to IR not supported {expression}")
