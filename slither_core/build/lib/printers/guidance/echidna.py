...
from slither_core.core.declarations import Enum, Function
from slither_core.core.declarations.solidity_variables import (
    SolidityVariableComposed,
    SolidityFunction,
    SolidityVariable,
)
from slither_core.core.expressions import NewContract
from slither_core.core.slither_core import SlitherCore
from slither_core.core.solidity_types import TypeAlias
...
from slither_core.visitors.expression.constants_folding import ConstantFolding
        ...
        if isinstance(ir, Binary):
            ...
            if isinstance(ir.variable_left, Constant) and isinstance(ir.variable_right, Constant):
                if ir.lvalue:
                    ...
                    all_cst_used.append(ConstantValue(
                        str(cst.value), str(type_)))
        if isinstance(ir, TypeConversion):
            if isinstance(ir.variable, Constant):
                if isinstance(ir.type, TypeAlias):
                    ...
                    value_type = ir.type
                all_cst_used.append(ConstantValue(
                    str(ir.variable.value), str(value_type)))
                continue
        if (
            ...
            try:
                internal_num = ir.variable_left.values.index(
                    ir.variable_right.value)
                all_cst_used.append(ConstantValue(
                    str(internal_num), "uint256"))
...
def _extract_constants(
    ...
    ret_cst_used_in_binary: Dict[str, Dict[str,
                                           Dict[str, List[ConstantValue]]]] = defaultdict(dict)
    for contract in slither.contracts:
        for function in contract.functions_entry_points:
            ...
            if all_cst_used:
                ret_cst_used[contract.name][_get_name(
                    function)] = list(set(all_cst_used))
