...
from slither_core.core.declarations.solidity_variables import SolidityVariableComposed
from slither_core.core.variables.state_variable import StateVariable
from slither_core.slither import Slither
from slither_core.slithir.operations.high_level_call import HighLevelCall
from slither_core.slithir.operations.index import Index
...
def visit_node(node, visited):
    ...
    taints = [v for v in taints if not isinstance(
        v, (TemporaryVariable, ReferenceVariable))]
    ...
    while set(prev_taints) != set(slither.context[KEY]):
        ...
        for contract in slither.contracts:
            for function in contract.functions:
                ...
                print(
                    f"All variables tainted : {[str(v) for v in slither.context[KEY]]}")
