...
from compile.source_unit import SafeDevSourceUnit
...
class SafeDevCompilationUnit:
    ...
    def __init__(self, safe_dev_compile: "SafeDevCompile", unique_id: str):
        ...
        self._source_units: Dict[Filename, SafeDevSourceUnit] = {}
        ...
        safe_dev_compile.compilation_units[unique_id] = self  # type: ignore

        self._unique_id = unique_id

    @property
    def unique_id(self) -> str: #done
    ...
    @property
    def crytic_compile(self) -> "SafeDevCompile": #done 
    ...
    @property
    def source_units(self) -> Dict[Filename, SafeDevSourceUnit]: #왜필요함?
        """
    ...
    def source_unit(self, filename: Filename) -> SafeDevSourceUnit: #일단 pass
        """
    ...
    def create_source_unit(self, filename: Filename) -> SafeDevSourceUnit:
        """
        ...
        if not filename in self._source_units:
            source_unit = SafeDevSourceUnit(self, filename)  # type: ignore
    ...
    def find_absolute_filename_from_used_filename(self, used_filename: str) -> str:
        ...
        if used_filename in self._safe_dev_compile.filenames:
