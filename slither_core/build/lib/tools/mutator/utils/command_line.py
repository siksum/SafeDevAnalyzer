...
from slither_core.tools.mutator.mutators.abstract_mutator import AbstractMutator
...
def output_mutators(mutators_classes: List[Type[AbstractMutator]]) -> None:
    ...
    table = MyPrettyTable(["Num", "Name", "What it Does",
                          "Fault Class", "Fault Nature"])

    # Sort by class, nature, name
    mutators_list = sorted(mutators_list, key=lambda element: (
