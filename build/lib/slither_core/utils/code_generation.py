...
from slither_core.core.declarations import Structure, Enum, Contract

if TYPE_CHECKING:
    from slither_core.core.declarations import FunctionContract, CustomErrorContract
...
def generate_interface_variable_signature(
    ...
    if unroll_structs:
        ...
        params = [p + " memory" if p in ["bytes",
                                         "string"] else p for p in params]
        returns = []
        _type = var.type
...
def generate_interface_function_signature(
    ...
    parameters = [format_var(param, unroll_structs)
                  for param in func.parameters]
    _interface_signature_str = (
        name + "(" + ",".join(parameters) + ") external" +
        payable + pure + view
    )
    if len(returns) > 0:
