...
from slither_core.core.solidity_types import TypeAlias, TypeAliasTopLevel, TypeAliasContract
from slither_core.core.solidity_types.array_type import ArrayType
from slither_core.core.solidity_types.elementary_type import (
    ElementaryType,
    ElementaryTypeName,
)
from slither_core.core.solidity_types.function_type import FunctionType
from slither_core.core.solidity_types.mapping_type import MappingType
...
from slither_core.solc_parsing.expressions.expression_parsing import CallerContextExpression

if TYPE_CHECKING:
    from slither_core.core.declarations import Structure, Enum, Function
...
def _find_from_type_name(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,too-many-arguments
    ...
    var_type = next(
        (c for c in contracts_direct_access if c.name == name_contract), None)

    if not var_type:
        var_type = next(
            (st for st in structures_direct_access if st.name == name), None)
    if not var_type:
        var_type = next(
            (e for e in enums_direct_access if e.name == name), None)
    if not var_type:
    ...
    if not var_type:
        ...
        if name_struct.startswith("struct "):
            ...
            name_struct = name_struct.split(" ")[0]
    ...
    if not var_type:
        var_type = next(
            (f for f in functions_direct_access if f.name == name), None)
    if not var_type:
        if name.startswith("function "):
            ...
            return_values = (
                [v for v in found[0][1].split(",") if v != ""] if len(
                    found[0]) > 1 else []
            )
            params = [
    ...
    if not var_type:
        if name.startswith("mapping("):
            ...
            if name.count("mapping(") == 1:
                found = re.findall(
                    r"mapping\(([a-zA-Z0-9\.]*) => ([ a-zA-Z0-9\.\[\]]*)\)", name)
            else:
                found = re.findall(
...
def parse_type(
    ...
    from slither_core.solc_parsing.variables.function_type_variable import FunctionTypeVariableSolc
    ...
    if isinstance(caller_context, SlitherCompilationUnitSolc) or (
        isinstance(caller_context,
                   FunctionSolc) and caller_context.contract_parser is None
    ):
        structures_direct_access: List["Structure"]
        ...
        caller_context,
        (StructureTopLevelSolc, CustomErrorSolc,
         TopLevelVariableSolc, UsingForTopLevelSolc),
    ):
