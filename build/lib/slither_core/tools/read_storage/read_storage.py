...
from slither_core.core.declarations import Contract, Structure
from slither_core.core.solidity_types import ArrayType, ElementaryType, MappingType, UserDefinedType
...
from slither_core.core.variables.structure_variable import StructureVariable
...
class SlitherReadStorage:
    ...
    def checksum_address(self) -> ChecksumAddress:
        ...
        if not self._checksum_address:
            self._checksum_address = self.web3.to_checksum_address(
        ...
        for contract, var in self.target_variables:
            ...
            if info:
                ...
                if isinstance(type_, UserDefinedType) and isinstance(type_.type, Structure):
                    ...
                    elems = self._all_array_slots(
                        var, contract, type_, info.slot)
    ...
    def convert_slot_info_to_rows(self, slot_info: SlotInfo) -> None:
        ...
        self.table.add_row([getattr(slot_info, field)
    ...
    def _find_mapping_slot(
        ...
        slot = keccak(encode([key_type, "uint256"], [
                      key, decode(["uint256"], slot)[0]]))
        ...
        if isinstance(target_variable_type.type_to, UserDefinedType) and isinstance(
            ...
            assert deep_key
            assert isinstance(
                target_variable_type.type_to.type_from, ElementaryType)
            ...
            assert isinstance(
                target_variable_type_type_to_type_to, (
